...
import type * as vd from './video-decoder';
...
export class VideoEncoder {
    ...
    this._p = this._p
      .then(async function () {
        ...
        if (supported) {
          ...
          // And initialize
          [self._codec, self._c, self._frame, self._pkt] = await libav.ff_init_encoder(
            supported.codec,
            supported
          );
      ...
      .then(async function () {
        ...
        try {
          ...
          for (let p = 0; p < planes; p++) {
            ...
            const hssf = vf.horizontalSubSamplingFactor(frameClone.format, p);
            const vssf = vf.verticalSubSamplingFactor(frameClone.format, p);
          ...
          // Possibly scale
          if (
            frame.width !== swsOut.width ||
            frame.height !== swsOut.height ||
            frame.format !== swsOut.format
          ) {
            if (frameClone._nonSquarePixels) {
              frame.sample_aspect_ratio = [frameClone._sar_num, frameClone._sar_den];
            }

            // Need a scaler
            let sws = self._sws;
            let swsIn = self._swsIn;
            let swsFrame = self._swsFrame;
            if (
              !sws ||
            ...
            const [, swsRes, , , , , , encRes] = await Promise.all([
            ...
            encodedOutputs = [];
            while (true) {
              const recv = await libav.avcodec_receive_packet(c, pkt);
              if (recv === -libav.EAGAIN) {
                break;
              } else if (recv < 0) {
                throw new Error('Encoding failed!');
              }
              encodedOutputs.push(await libav.ff_copyout_packet(pkt));
            }
          } else {
            if (this._nonSquarePixels) {
              frame.sample_aspect_ratio = [this._sar_num, this._sar_den];
            }

            // Encode directly
            encodedOutputs = await libav.ff_encode_multi(c, framePtr, pkt, [frame]);
          }
  ...
  private async _getExtradata() {
    ...
    if (extradata && extradata_size) {
      this._metadata.decoderConfig.description = this._extradata = await libav.copyout_u8(
        extradata,
        extradata_size
      );
    }
  ...
  private _outputEncodedVideoChunks(packets: LibAVJS.Packet[]) {
    ...
    for (const packet of packets) {
      // 1. type
      const type: evc.EncodedVideoChunkType = packet.flags & 1 ? 'key' : 'delta';
  ...
  flush(): Promise<void> {
    ...
    const ret = this._p.then(async () => {
      ...
      try {
        encodedOutputs = await libav.ff_encode_multi(c, frame, pkt, [], true);
  ...
  static async isConfigSupported(config: VideoEncoderConfig): Promise<VideoEncoderSupport> {
    ...
    return {
      supported,
      config: misc.cloneConfig(config, [
        'codec',
        'width',
        'height',
        'bitrate',
