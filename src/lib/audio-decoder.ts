...
export class AudioDecoder {
  ...
  configure(config: AudioDecoderConfig): void {
    ...
    if (this._libav) this._p = this._p.then(() => this._free());
  ...
  decode(chunk: eac.EncodedAudioChunk) {
    ...
    this._p = this._p
      .then(async function () {
         ...
         * decoded outputs. */
        if (decodedOutputs) self._outputAudioData(decodedOutputs);
      })
      .catch(this._error);
  }
  ...
  private _outputAudioData(frames: LibAVJS.Frame[]) {
    ...
    for (const frame of frames) {
      ...
      if (planar) {
        let ct = 0;
        for (let i = 0; i < frame.data.length; i++) ct += frame.data[i].length;
