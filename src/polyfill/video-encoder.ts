...
export class VideoEncoder {
  ...
  configure(config: VideoEncoderConfig): void {
    ...
    this._p = this._p.then(async function () {
      ...
      if (supported) {
        ...
        // And initialize
        [self._codec, self._c, self._frame, self._pkt] = await libav.ff_init_encoder(supported.codec, supported);
        ...
        if (frame.width !== swsOut.width
          ...
          if (!sws
            ...
            sws = await libav.sws_getContext(
              swsIn.width, swsIn.height, swsIn.format, swsOut.width, swsOut.height, swsOut.format, 2, 0, 0, 0,
            );
            self._sws = sws;
            self._swsIn = swsIn;
  ...
  private async _getExtradata() {
    ...
    if (extradata && extradata_size) {
      this._metadata.decoderConfig.description = this._extradata = await libav.copyout_u8(extradata, extradata_size);
    }
