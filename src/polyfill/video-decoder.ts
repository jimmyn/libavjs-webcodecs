  ...
  configure(config: VideoDecoderConfig): void {
    ...
    this._p = this._p.then(async () => {
      ...
      if (supported) {
        ...
        // Initialize
        [self._codec, self._c, self._pkt, self._frame] = await libav.ff_init_decoder(supported.codec);
        const ptr = libav.malloc(config.description.byteLength);
        libav.HEAPU8.set(config.description, ptr);
        await libav.AVCodecContext_extradata_s(self._c, ptr);
        await libav.AVCodecContext_extradata_size_s(self._c, config.description.byteLength);
