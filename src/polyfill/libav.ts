...
async function codecs(encoders: boolean): Promise<string[]> {
  ...
  for (const [avname, codec] of [
    ...
    } else if (await libav.avcodec_find_decoder_by_name(avname)) {
      ret.push(codec);
    }
  }
...
function av1Advanced(codecParts: string[], ctx: LibAVJS.AVCodecContextProps) {
  if (codecParts[1]) {
    const profile = Number(codecParts[1]);
  ...
  if (codecParts[2]) {
    const level = Number(codecParts[2]);
    if (level >= 0 && level <= 23) {
      ctx.level = level;
    } else {
  ...
  if (codecParts[4]) {
    const depth = Number(codecParts[3]);
...
function vp9Advanced(codecParts: string[], ctx: LibAVJS.AVCodecContextProps) {
  ...
  if (codecParts[2]) {
    const level = [Number(codecParts[2][0]), Number(codecParts[2][1])];
  ...
  if (codecParts[4]) {
    const chromaMode = Number(codecParts[4]);
    switch (chromaMode) {
      case 0:
      case 1:
